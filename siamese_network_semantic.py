...
class SiameseLSTMw2v(object):
    ...
    def __init__(
        ...
        self.extra_feature=tf.placeholder(tf.float32,[None],name='extra_feature')
        ...
        with tf.name_scope("midoutput"):
            ...
            self.out2=self.stackedRNN(self.embedded_words2, self.dropout_keep_prob, "side1", embedding_size, sequence_length, hidden_units,unit_type)

        with tf.name_scope('output'):
            print('output var scope :{}'.format(tf.get_variable_scope().name))
            self.mid_output = tf.abs(tf.subtract(self.out1, self.out2))
            self.reshape_feature=tf.reshape(self.extra_feature,[-1,1],name='reshape_feature')
            self.mid_output = tf.concat([self.mid_output, self.reshape_feature], 1)
            self.mid_output = tf.layers.batch_normalization(self.mid_output, training=training)
            self.mid_output = tf.layers.dense(self.mid_output, 1, activation=tf.nn.sigmoid, name='sigmoid')
            self.output = tf.reshape(self.mid_output, [-1], name='out')
